...
위치 공유는 기본으로 꺼져 있습니다. 함께 보기에서 켜면 자유시간에도 공유하고, 꺼도 자유시간·이동 중 같은 상태는 유지됩니다. 도착 뒤에도 켜 둔 공유는 유지되며 모임 종료 시 모든 참가자 공유가 종료됩니다. 공유를 끄면 이 기기의 백그라운드 위치 수집을 즉시 중지하고 서버에서 좌표를 제거합니다. 오프라인이면 상대에게 변경 전달이 지연될 수 있어 화면에 전달 대기를 표시합니다.
...
현재 앱은 동봉한 Node 중계 서버 방식입니다. 자체 서버 운영을 없애려면 Firebase/CloudKit 같은 관리형 서비스로 전환할 수 있지만, 이 프로젝트는 아직 그 방식으로 전환하지 않았습니다. 근거리 무선 연결만으로 멀리 떨어진 여러 기기의 인터넷 동기화를 대신하지 않습니다.
...
기본 포트는 8787입니다. 실제 현장에서는 HTTPS 주소로 배포한 뒤 `REUNION_SERVICE_URL`에 설정하고 같은 구성으로 여러 기기에 설치합니다. 로컬 실행만으로 인터넷에 공개되지는 않습니다. 서버 미설정 시 앱은 초대 코드 만들기·참여를 비활성화합니다.

A가 장소와 시간을 정한 뒤 친구와 연결에서 초대 코드를 만들고, 친구들이 같은 코드로 참여합니다. 각자 함께 보기에서 위치 공유를 직접 켭니다. 연결 후 약속 장소·시간은 고정되며 각자의 이동수단·여유시간은 재합류 화면에서 변경할 수 있습니다.

서버는 메모리 저장소로 재시작 시 모임이 사라집니다. 12시간 뒤 다음 요청 시 정리합니다. 최대 10명 제한, 참여자별 인증 토큰, 요청 크기·빈도 제한이 있습니다. 검색과 경로 계산 시에는 현재 위치가 Google에 전달됩니다.
...
한 모임에는 본인을 포함해 최대 10명이 같은 초대 코드로 참여할 수 있습니다. 함께 보기에서 각 참가자의 상태와 동의한 위치를 따로 표시합니다. 출발 알림은 나머지 참가자 전원에게 전달하며, 라이브 액티비티는 친구들의 이동·도착 인원을 요약합니다. 모임 종료는 모든 참가자의 위치 공유를 중지합니다.

현장 사용에는 변경된 서버를 HTTPS로 배포하고 모든 iPhone에 동일한 서버 주소를 설정해야 합니다. APNs 설정이 없으면 백그라운드 출발 푸시는 전송되지 않습니다. 자동 검증은 서버 테스트 클라이언트와 시뮬레이터 기준이며, 실제 여러 iPhone 현장 검증은 별도로 필요합니다.
